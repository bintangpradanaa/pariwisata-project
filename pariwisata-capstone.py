...
st.markdown('''<div style="font-size: 16px; text-align: center; margin-top: 50px; margin-bottom: 40px; margin-right:20px; margin-left:20px;">
           Indonesia telah diakui sebagai negara paling indah di dunia menurut daftar "The Most Beautiful Countries" oleh Forbes, dengan skor keindahan alam 7.77 
           dari 10. Dengan memiliki 31.000 mil persegi terumbu karang yang sebagian besar bisa dieksplorasi dari provinsi populer Bali, Taman Nasional Komodo, 
           hutan tropis di Sumatera, dan cagar alam orangutan di Kalimantan menjadi tempat penting yang mendukung reputasi Indonesia sebagai destinasi pariwisata 
           terbaik. Namun, di balik pesona alamnya, sektor pariwisata Indonesia menghadapi tantangan dan potensi yang perlu dipertimbangkan secara cermat. Untuk 
           memahami potensi sektor pariwisata Indonesia lebih dalam, penting untuk meninjau kontribusinya terhadap ekonomi, jumlah kunjungan wisatawan, tingkat hunian 
           hotel, perbandingan dengan negara-negara tetangga di ASEAN, serta potensi pariwisata di Indonesia. Dengan pemahaman yang mendalam terhadap dinamika industri 
           pariwisata, diharapkan kita dapat memiliki pandangan yang lebih komprehensif mengenai kondisi dan prospek masa depan pariwisata Indonesia. 
...
eco_head_col, _,  target_devisa_col = st.columns([1.6, 0.1, 1.3])
with eco_head_col:
    st.subheader('Kontribusi Pariwisata terhadap Ekonomi')
    st.markdown('''<p style = "font-size:15px; text-align:center; font-weight:bold;">
                Grafik 1. Devisa Pariwisata dan Kontribusi Pariwisata terhadap PDB </p>''', unsafe_allow_html=True)
with target_devisa_col:
    st.write("")
    ...
    with target_2023_eco_col:
        st.metric('Target Minimum Devisa 2023', f'{target_minimum_devisa_2023}B USD')
    ...
    with target_2024_eco_col:
        st.metric('Target Minimum Devisa 2024', f'{target_devisa_2024}B USD')
        

pariwisata_eco,_, isi_eco = st.columns([1.6, 0.1, 1.3])
with pariwisata_eco:
    st.write('')
...
with isi_eco:
    ...
    with capaian_devisa_2024_col:
        capaian_devisa_2024_color = "green" if capaian_devisa_2024 != 'No Data' else "grey"
...
st.write('''<p style = "font-size:15px; text-align:justify;">
        Dalam rentang tahun 2017 hingga 2019, sektor pariwisata Indonesia menunjukkan pertumbuhan yang stabil, baik dalam hal pendapatan devisa 
        maupun kontribusinya terhadap Produk Domestik Bruto (PDB). Namun, ketika pandemi melanda pada tahun 2020, sektor pariwisata mengalami 
        dampak yang sangat signifikan. Pada puncaknya, yaitu tahun 2021, pendapatan devisa dari pariwisata hanya mencapai 520 juta USD. 
        <span style="font-weight:bold;"> Menariknya, angka kontribusi pariwisata terhadap PDB di tahun tersebut mengalami kenaikan yang tidak 
        sebanding dengan devisa yang diperoleh.</span> Meskipun demikian, seiring berjalannya waktu, sektor pariwisata mulai perlahan naik dan menunjukkan 
...
wisman_head_col, _,  target_wisman_col = st.columns([1.6, 0.1, 1.3])
...
with target_wisman_col:
    ...
    with target_2023_wisman_col:
        st.metric('Target Minimum Wisman 2023', format_big_number(target_wisman_2023))
    
    with target_2024_wisman_col:
        st.metric('Target Minimum Wisman 2024', f'{target_wisman_2024} M')
    
wisman_pertahun,_, isi_wisman_pertahun = st.columns([1.6, 0.1, 1.3])
with wisman_pertahun:
    ...
    fig_bar_wisman_pertahun.update_layout(width=800, height=380, margin=dict(t=0))
    st.plotly_chart(fig_bar_wisman_pertahun, use_container_width=True)
    st.markdown(""" <div style="position: absolute; top: -40px; right: 0px; color: #808080; font-size: 13px;">
            (Sumber: <a href="https://www.bps.go.id/id/statistics-table?subject=561" style="text-decoration:none;">BPS Indonesia</a>)</div>""", unsafe_allow_html=True)
    
    
with isi_wisman_pertahun:
    capaian_wisman_2023_col, capaian_wisman_2024_col = st.columns(2)
    with capaian_wisman_2023_col:
    ...
    with capaian_wisman_2024_col:
        capaian_wisman_2024_color = "green" if capaian_wisman_2024 != 'No Data' else "grey"
        st.write(f'Capaian: <span style="color: {capaian_wisman_2024_color};">{capaian_wisman_2024}</span>', unsafe_allow_html=True)
  
    st.write("")
    st.write('''<p style= "font-size:15px; text-align:justify;">
            Walaupun masih berada dalam fase pemulihan pasca-pandemi, pencapaian ini menandakan kemajuan yang luar biasa. Dengan capaian melebihi target, 
            prestasi ini memberikan bukti konkret akan keberhasilan upaya yang telah dilakukan dan ini harus dipertahankan.</p>''', unsafe_allow_html=True)
    st.markdown('''<div style="border: 1px solid; padding: 10px; border-radius: 5px; font-size:14px; text-align:justify; padding:15px; font-style:italic;">
            Untuk mencapai target selanjutnya, dapat dilakukan dengan memperkuat digitalisasi dalam sektor pariwisata meliputi peningkatan 
            ...
            penerbangan dan agen perjalanan.</div>''', unsafe_allow_html=True)
...
with wisman_pernegara:  
    ...
    fig_bar_negara.update_layout(width=600, height=380, margin=dict(t=30))
...
with wisman_pintu_masuk:
    ...
    st.markdown('''<p style = "font-size:15px; text-align:center; top:50px;font-weight:bold;">
               Grafik 4. Jumlah Kunjungan Wisman per Pintu Masuk</p>''', unsafe_allow_html=True)
    ...
    fig_bar_pintu.update_layout(width=800, height=380, margin=dict(t=30))
...
st.markdown('''<p style="font-size:15px; text-align:justify;">
            ...
            ke Indonesia melalui berbagai jalur masuk, termasuk udara, darat, dan laut. <span style="font-weight:bold;">Meskipun jalur udara secara historis menjadi 
            opsi utama, terjadi penurunan yang signifikan dalam kunjungan melalui jalur udara saat pandemi.</span> Sebaliknya, jalur darat, mendominasi sebagai jalur 
...
with wisman_most:
    ...
    st.markdown('''<p style = "font-size:15px; text-align:center; top:100px;font-weight:bold;">
                Grafik 5. Top 10 Provinsi Tujuan Wisman </p>''', unsafe_allow_html=True)
    wisman_bar_perprov = px.bar(top10_wisman_prov,  
                                x='jumlah_kunjungan_wisman',
                                y='provinsi',
                                color= 'provinsi',
                                labels={'jumlah_kunjungan_wisman': 'Jumlah Kunjungan Wisman', 'provinsi': 'Provinsi'})
    wisman_bar_perprov.update_layout(width=800, height=380, showlegend=False, margin=dict(t=40))
...
# Header: Wisnus
wisnus_head_col, _,  target_wisnus_col = st.columns([1.6, 0.1, 1.3])
with wisnus_head_col:
    st.subheader("Tren Kunjungan Wisatawan Nusantara")
    st.markdown('''<p style = "font-size:15px; text-align:center;font-weight:bold;">
                Grafik 6. Jumlah Kunjungan Wisnus per Tahun </p>''', unsafe_allow_html=True)
with target_wisnus_col:
    ...
    with target_2023_wisnus_col:
        st.metric('Target Minimum Wisnus 2023', format_big_number(target_wisnus_2023))
        
    with target_2024_wisnus_col:
        st.metric('Target minimum Wisnus 2024',format_big_number(target_wisnus_2024))
        
wisnus_pertahun, _, isi_wisnus_pertahun = st.columns([1.6, 0.1, 1.3])
with wisnus_pertahun:
    ...
    wisnus_bar_pertahun.update_layout(width=600, height=400, yaxis=dict(dtick=1e8, range=[0, 9e8]), margin=dict(t=0))
    st.plotly_chart(wisnus_bar_pertahun, use_container_width=True)
    st.markdown(""" <div style="position: absolute; top: -40px; right: 0px; color: #808080; font-size: 13px;">
            (Sumber: <a href="https://www.bps.go.id/id/statistics-table?subject=561" style="text-decoration:none;">BPS Indonesia</a>)</div>""", unsafe_allow_html=True)

with isi_wisnus_pertahun:
    capaian_wisnus_2023_col, capaian_wisnus_2024_col = st.columns(2)
    with capaian_wisnus_2023_col:
    ...
    with capaian_wisnus_2024_col:
        capaian_wisnus_2024_color = "green" if capaian_wisnus_2024 != 'No Data' else "grey"
    ...
    st.markdown('''<p style="font-size:15px; text-align:justify;">
            ...
            mencapai 62.43% dari target tersebut. <span style="font-weight:bold">Hal ini merupakan dampak lanjutan dari pandemi, termasuk pembatasan perjalanan, 
            ketidakmampuan daya beli masyarakat, dan ketidakstabilan ekonomi</span> menjadi faktor utama yang mempengaruhi minat dan kemampuan 
            masyarakat untuk melakukan perjalanan.</p>''', unsafe_allow_html=True)
    st.markdown('''<div style="border: 1px solid; padding: 10px; border-radius: 5px; font-size:14px; text-align:justify; padding:15px; font-style:italic;">
            Dari sini, pentingnya kolaborasi antara lembaga pemerintahan, seperti Kemenparekraf dengan Kementerian Perhubungan (Kemenhub) 
            untuk memfasilitasi akses transportasi dan menekan harga tiket pesawat, layanan kereta api, dan transportasi lainnya.
            </div>''', unsafe_allow_html=True)
...
st.info('''
        ...
        Hal ini terlihat dalam **[Grafik 1](#kontribusi-pariwisata-terhadap-ekonomi-indonesia)**, **yang menunjukkan kontribusi pariwisata terhadap 
        Produk Domestik Bruto (PDB) mengalami kenaikan dan salah satu faktornya adalah efek dari meningkatnya jumlah perjalanan wisatawan 
        nusantara, yang secara langsung meningkatkan pengeluaran di sektor pariwisata.**_

        _Penurunan kunjungan wisman, telah mendorong pemerintah dan pelaku industri pariwisata untuk memperkuat promosi dan pengembangan destinasi 
        wisata dalam negeri. Dengan ini, wisnus merasa lebih tertarik untuk mengeksplorasi destinasi dalam negeri, yang pada akhirnya dapat meningkatkan
        kontribusi sektor pariwisata terhadap PDB._ 
        
        _**Meskipun demikian, pengaruh yang paling signifikan terhadap devisa pariwisata masih ditentukan 
        oleh jumlah kunjungan wisman.** Hal ini tergambar dari **[Grafik 1](#kontribusi-pariwisata-terhadap-ekonomi-indonesia)** dan **[Grafik 2](#tren-dan-profile-kunjungan-wisatawan-mancanegara)**, 
...
with wisnus_provinsi:
    ...
    st.markdown('''<p style = "font-size:15px; text-align:center;font-weight:bold;">
                Grafik 7. Top 10 Provinsi dengan Kunjungan Wisnus Terbanyak </p>''', unsafe_allow_html=True)    
    fig_bar_wisnus_prov = px.bar(top10_wisnus_prov,
                                x='jumlah_kunjungan_wisnus',
                                y='provinsi',
                                color='provinsi',
                                labels={'jumlah_kunjungan_wisnus': 'Jumlah Kunjungan Wisnus', 'provinsi': 'Provinsi'})
    fig_bar_wisnus_prov.update_layout(width=800, height=400, showlegend=False, margin=dict(t=30))
...
with wisnus_prov_perbulan:
    ...
    fig_line_wisnus_prov.update_layout(width=800, 
                                       height=400, 
                                       xaxis={'title': {'text': 'Bulan', 'standoff': 20}, 'tickangle': 0}, margin=dict(t=30))
    st.plotly_chart(fig_line_wisnus_prov, use_container_width=True)
    st.markdown(""" <div style="position: absolute; top: -40px; right: 0px; color: #808080; font-size: 13px;">
            (Sumber: <a href="https://www.bps.go.id/id/statistics-table?subject=561" style="text-decoration:none;">BPS Indonesia</a>, 2019 - Nov2023)</div>""", unsafe_allow_html=True)
...
# Header: TPK Hotel
st.write('')
st.subheader('Tingkat Penghunian Kamar Hotel')
st.markdown("<p style='color: gray; font-style: italic;'>Compared to Last Year</p>", unsafe_allow_html=True)
...
with tpk_hotel_pertahun_col:
    ...
    st.write("")
    st.markdown('''<p style = "font-size:15px; text-align:center;font-weight:bold;">
                Grafik 9. Tren Tingkat Penghunian Kamar Hotel Bintang dan Non Bintang</p>''', unsafe_allow_html=True)
    ...
    fig_line_hotel_pertahun.update_layout(width=800, height=400, yaxis=dict(dtick=5, range=[0, 60]), margin=dict(t=30))
...
tpk_hotel_perprov_col, _, hotel_col = st.columns([1.55, 0.1, 1.35])
with tpk_hotel_perprov_col:
    ...
    st.markdown('''<p style = "font-size:15px; text-align:center;font-weight:bold;">
                Grafik 10. Top 5 Provinsi dengan TPK Tertinggi</p>''', unsafe_allow_html=True)
    tpk_bar_perprov = px.bar(top5_tpk_prov,  
                            y='provinsi', 
                            x=top5_tpk_prov.columns[1],  
                            ...
                            color='provinsi')
    tpk_bar_perprov.update_layout(width=800, height=400, xaxis=dict(dtick=4, range=[20, 80]), margin=dict(t=30))  
...
with hotel_col:
    ...
    st.markdown('''<p style = "font-size:15px; text-align:center;font-weight:bold; top:40px;">
                Grafik 11. Top 5 Jumlah Hotel dan Akomodasi Terbanyak per Provinsi</p>''', unsafe_allow_html=True)
    fig_bar_hotel = px.bar(top5_data_hotel, 
                x='jumlah_hotel',
                y='provinsi',
                color='provinsi',
                labels={'jumlah_hotel': 'Jumlah Hotel dan Akomodasi Lainnya', 'provinsi': 'Provinsi'})
    fig_bar_hotel.update_layout(width=800, height=400, margin=dict(t=30))
...
st.write('''<p style="font-size:15px; text-align:justify;">
        ...
        itu, DKI Jakarta tetap menjadi yang tertinggi untuk hotel non-bintang, menunjukkan dominasi bisnis perhotelan di ibu kota yang stabil.\

        <p style="font-size:15px; text-align:justify;">
        <span style="font-weight:bold;">Pada tahun 2018, DKI Jakarta mencatat tingkat penghunian kamar tertinggi mencapai 66.87% pada hotel Bintang 
        dan 74,27% pada hotel non bintang, yang merupakan angka tertinggi dalam delapan tahun terakhir.</span> Salah satu faktor pengaruhnya adalah 
        adanya event besar seperti Asian Games dan event-event lain yang mendorong tingkat hunian hotel di provinsi tersebut. Fenomena serupa juga 
        terjadi di Provinsi 
        ...
        provinsi-provinsi di Pulau Jawa secara konsisten berada di lima besar. <span style="font-weight:bold">Hal ini menyoroti adanya hubungan yang erat antara 
        infrastruktur akomodasi dengan jumlah kunjungan wisatawan, karena ketersediaan akomodasi yang memadai sangat penting bagi para wisatawan. </span>
        </p></p></p>''', unsafe_allow_html=True)
st.info('''
...
st.write('')
st.subheader("Perbandingan Wisatawan Asing di Negara ASEAN")
...
with asean_data_col:
    ...
    st.markdown('''<p style = "font-size:15px; text-align:center;font-weight:bold; top:40px;">
                Grafik 12. Jumlah kunjungan Wisatawan di Berbagai Negara ASEAN</p>''', unsafe_allow_html=True)
    ...
    st.markdown(""" <div style="position: absolute; top: -40px; right: 0px; color: #808080; font-size: 13px;">
            (Sumber: <a href="https://data.aseanstats.org/" style="text-decoration:none;">ASEANStats</a>)</div>""", unsafe_allow_html=True)
...
st.subheader('Potensi Pariwisata Indonesia')
...
# title map
st.write('')
st.markdown('<h5 style="text-align: center;">Peta Sebaran Jenis dan Objek Daya Tarik Wisata Indonesia</h5>', unsafe_allow_html=True)
# show the map
...
st.write('''<p style="font-size:15px; text-align:justify;">
        ...
        memang sepantasnya menjadi pusat utama dari berbagai daya tarik wisata di Indonesia. Apabila hal ini dapat dimanfaatkan dengan baik, potensi ini dapat menarik 
        lebih banyak lagi wisatawan khususnya Wisatawan Mancanegara. </span>''', unsafe_allow_html=True)
st.info('''
        _Untuk mendukung pertumbuhan pariwisata yang berkelanjutan, infrastruktur pendukung yang memadai sangatlah penting, termasuk akomodasi, transportasi, dan sarana 
        penunjang lainnya. Tidak hanya untuk wilayah Pulau Jawa, tetapi juga untuk daerah-daerah lain yang memiliki potensi pariwisata yang besar seperti Sumatera Utara, 
        Kalimantan Barat, Sulawesi Utara, dan wilayah lainnya. Dengan upaya ini, pengalaman wisatawan yang mengunjungi berbagai destinasi di seluruh Indonesia dapat ditingkatkan._

        _**Penting juga untuk mengembangkan SDM pariwisata yang berkualitas serta meningkatkan digitalisasi dan promosi pariwisata secara agresif. Hal ini bertujuan agar potensi 
        pariwisata di Indonesia dapat dioptimalkan sepenuhnya. Dengan memanfaatkan potensi ini secara maksimal, sektor pariwisata dapat memberikan kontribusi yang lebih besar 
        terhadap perekonomian nasional.**_''')
